...
ID3D12DescriptorHeap* CreateDescriptorHeap(
	ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT numDescriptors, bool shaderVisble)
{
...
DirectX::ScratchImage LoadTexture(const std::string& filePath)
{
	...
	// ミニマップの作成
	DirectX::ScratchImage mipmages{};
	hr = DirectX::GenerateMipMaps(image.GetImages(), image.GetImageCount(), image.GetMetadata(), DirectX::TEX_FILTER_SRGB, 0, mipmages);
	assert(SUCCEEDED(hr));
...
#pragma region DirectX12のテクスチャリソースを作る関数
...
#pragma region TextureResourceにデータを転送する関数
...
#pragma region SwapChain
	...
	swapChainDesc.SampleDesc.Count = 1; // マルチサンブルしない
...
#pragma region SwapChainResources
	//SwapChaiからResourceを引っ張ってくる
...
#pragma endregion
	...
	ID3D12Resource* vertexResources = CreateBufferResource(device, sizeof(VertexData) * 3);


	// 頂点バッファビューを作成する
	D3D12_VERTEX_BUFFER_VIEW vertexBufferView{};
	// リソースの先頭のアドレスから使ラ
	vertexBufferView.BufferLocation = vertexResources->GetGPUVirtualAddress();
	...
	// 書さ込むためのアドレスを取得
	vertexResources->Map(0, nullptr, reinterpret_cast<void**>(&vertexData));
	// 左下
	...
	const DirectX::TexMetadata& metadata = mipImages.GetMetadata();
	ID3D12Resource* textureResources = CreateTextureResource(device, metadata);
	UploadTextureData(textureResources, mipImages);
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*wvpDate = worldViewProjectionMatrix;
			...
			// GetCompletedValueの初期値はFence作成時に渡した初期化
			if (fence->GetCompletedValue() < fenceValue)
			{
				// 指定したSignalにたどり着いていないので、だどりつくまで待つようにイベントを指定する
